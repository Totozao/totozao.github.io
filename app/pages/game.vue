...
type MasterRoleEntry = { names: string[]; allDead: boolean };

const masterRoleGroups = computed(() => {
    const groups: Record<string, MasterRoleEntry> = {};
    for (const p of playersInfo.activePlayers) {
        const role = p.role || "civilian";
        if (!groups[role]) groups[role] = { names: [], allDead: false };
        groups[role].names.push(p.lives > 0 ? p.name : `${p.name} (мёртв)`);
    }
    for (const role of Object.keys(groups)) {
        groups[role].allDead = playersInfo.activePlayers
            .filter(p => (p.role || "civilian") === role)
            .every(p => p.lives <= 0);
    }
    return groups;
});

const currentNightRoleIrrelevant = computed(() => {
    if (!currentNightRole.value) return false;
    const role = currentNightRole.value;
    if (role === "mafia") {
        return playersInfo.activePlayers.filter(p => p.role === "mafia" && p.lives > 0).length === 0;
    }
    return !playersInfo.activePlayers.some(p => p.role === role && p.lives > 0);
});
...
const currentActiveRolesForNight = computed(() => {
    return nightRoleOrder.filter((r) => {
        if (
            !playersInfo.activeRoles.includes(r) ||
            playersInfo.isRoleInactive({ value: r })
        ) {
            return false;
        }

        if (r === "patrol") {
            const detectives = playersInfo.activePlayers.filter(
                (p) => p.role === "detective",
            );
            const isDetectiveDeadOrGone =
                detectives.length === 0 ||
                detectives.every((p) => p.lives <= 0);
...
const isSectarianNightRole = computed(
    () => currentNightRole.value === "sectarian",
);
...
const nextNightRole = () => {
    ...
    if (
        ...
        currentActiveRolesForNight.value.length - 1
    ) {
        currentNightRoleIndex.value++;
    } else {
        finishNight();
...
const executeNightAction = (target1?: string, target2?: string) => {
    ...
    const actor = isMafiaNightRole.value
        ? "Мафия"
        : currentRoleActor.value?.name || "Неизвестный";
...
const skipNightAction = () => {
    ...
    const actor = isMafiaNightRole.value
        ? "Мафия"
        : playersInfo.activePlayers.find((p) => p.role === role && p.lives > 0)
...
const getRoleText = (roleKey: string) => {
    const texts: Record<string, string> = {
        ...
        sectarian:
            "Сектант: выберите игрока для вербовки или пропустите выбор цели",
...
const finishVotingPhase = (
    kickedPlayer?: string,
    wasSavedByFortune: boolean = false,
) => {
    if (kickedPlayer && !wasSavedByFortune) {
        const p = playersInfo.activePlayers.find(
            (p) => p.name === kickedPlayer,
        );
        if (p) {
            p.lives = 0;
...
const resolveVoting = () => {
    ...
    if (maxVotes === 0 || isTie || !kickedPlayer) {
        ...
        // Circle of Fortune
        const circleRounds =
            parseInt(playersInfo.circleOfFortuneRounds.toString()) || 2;
        if (playersInfo.currentNight <= circleRounds) {
            const alivePlayers = playersInfo.activePlayers.filter(
                (p) => p.lives > 0,
            );
            const savedPlayer =
            ...
            const spin = () => {
                if (spins < maxSpins) {
                    ...
                    setTimeout(() => {
                        const wasKickedPlayerSaved =
                            savedPlayer?.name === kickedPlayer;

                        if (wasKickedPlayerSaved) {
                            toast.success(
                                `Колесо Фортуны спасло игрока: ${kickedPlayer}! Никто не изгнан.`,
                                "Колесо Фортуны",
                                6000,
                            );
                        } else {
                            toast.info(
                                `Колесо Фортуны выбрало игрока: ${savedPlayer?.name || ""}. ${kickedPlayer} изгнан голосованием.`,
                                "Колесо Фортуны",
                                6000,
                            );
                        }
...
const checkWinConditions = () => {
    ...
    if (alive.length === 2) {
        const hasDetectiveOrPatrol = alive.some(
            (p) => p.role === "detective" || p.role === "patrol",
        );
        const hasNegative = alive.some((p) => isNegative(p.role));

        if (hasDetectiveOrPatrol && hasNegative) {
            winnerMessage.value =
                "Победа Мирных (Шоудаун)! Представитель закона одолел преступника 1 на 1.";
            playersInfo.currentGameStep = "day";
            return;
        }
    }
...
useHead({
    ...
    meta: [
        {
            name: "description",
            content: "Играйте в Мафию и узнайте, кто из вас мафия",
        },
    ],
});
</script>
...
<template>
    <NuxtLayout>
        <div
            class="flex flex-col items-center gap-8 w-full max-w-lg mx-auto py-8"
        >
            <div
                v-if="winnerMessage"
                class="text-center space-y-4 w-full bg-neutral-900 border border-green-500 rounded-2xl p-6 shadow-[0_0_30px_rgba(34,197,94,0.3)]"
            >
            ...
            <template v-else>
                ...
                <template v-if="isNightPhase">
                    <div class="text-center space-y-2">
                        <h1
                            class="text-3xl font-bold bg-gradient-to-r from-blue-500 to-indigo-400 bg-clip-text text-transparent"
                        >
                            Ночь {{ playersInfo.currentNight + 1 }}
                        </h1>
                        <p class="text-neutral-400 font-medium">
                            Просыпаются ночные жители...
                        </p>
                    </div>
                    ...
                    <ClientOnly>
                        ...
                        >
                            <div
                                class="absolute inset-0 bg-gradient-to-br from-blue-500/10 via-indigo-500/5 to-purple-500/10 pointer-events-none"
                            ></div>

                            <Transition name="role-card" mode="out-in">
                                <div
                                    :key="currentNightRole"
                                ...
                                >
                                    ...
                                    >
                                        <div class="space-y-2">
                                            <p
                                                class="text-xs uppercase tracking-[0.3em] text-blue-300/70"
                                            >
                                                Текущий ход
                                            </p>
                                            <h2
                                            ...
                                            <p class="text-neutral-400">
                                                {{
                                                    currentNightRole
                                                        ...
                                                        : "Нет активных ролей для этой ночи."
                                                }}
                                            </p>
                                            <p
                                                v-if="
                                                    isMafiaNightRole &&
                                                    mafiaTeamMembersText
                                                "
                                                class="w-full rounded-xl border border-emerald-400/20 bg-emerald-400/10 px-3 py-1 text-sm font-medium text-emerald-200"
                                            >
                                                Мафия действует командой:
                                                {{ mafiaTeamMembersText }}.
                                                Выберите одну общую жертву.
                                            </p>
                                            <p
                                                v-else-if="currentRoleActor"
                                                class="w-full rounded-xl border border-emerald-400/20 bg-emerald-400/10 px-3 py-1 text-sm font-medium text-emerald-200"
                                            >
                                                {{ currentRoleActor.name }} на
                                                роли
                                                {{
                                                    getRoleTitle(
                                                        currentNightRole,
                                            ...
                                            <p
                                                v-if="playersInfo.isMasterMode && currentNightRoleIrrelevant"
                                                class="w-full rounded-xl border border-amber-400/30 bg-amber-400/10 px-3 py-1 text-sm font-medium text-amber-300"
                                            >
                                                Игрок с этой ролью мёртв или отсутствует — выбор не имеет значения.
                                            </p>
                                        </div>
                                        <span
                                            class="rounded-full border border-blue-400/20 bg-blue-400/10 px-3 py-1 text-sm font-semibold text-blue-200"
                                        >
                                            {{ currentNightRoleIndex + 1 }}/{{
                                                Math.max(
                                                    currentActiveRolesForNight.length,
                                                    1,
                                                )
                                            }}
                                        </span>
                                    ...
                                    >
                                        <SharedUiButton
                                            ...
                                            @click="
                                                isNightActionModalVisible = true
                                            "
                                        />
                                        <SharedUiButton
                ...
                <template v-else-if="isDayPhase">
                    <div class="text-center space-y-2">
                        <h1
                            class="text-3xl font-bold bg-gradient-to-r from-yellow-500 to-orange-400 bg-clip-text text-transparent"
                        >
                            День {{ playersInfo.currentNight }}
                        </h1>
                        <p class="text-neutral-400 font-medium">
                            Город просыпается. Обсуждение и голосование.
                        </p>
                    </div>

                    <div
                        class="w-full bg-neutral-900 border border-neutral-800 rounded-2xl p-6 shadow-xl text-center flex flex-col gap-4"
                    >
                        <h2 class="text-xl font-bold text-neutral-200">
                            Дневное обсуждение
                        </h2>
                        <p class="text-neutral-400">
                            Живые игроки: {{ playersInfo.activePlayers.length }}
                        </p>
                ...
                <template v-else-if="playersInfo.currentGameStep === 'voting'">
                    ...
                    <div
                        v-if="showFortuneWheel"
                        class="text-center space-y-4 w-full bg-neutral-900 border border-purple-500 rounded-2xl p-8 shadow-[0_0_30px_rgba(168,85,247,0.3)] flex flex-col items-center justify-center min-h-[350px]"
                    >
                        <h1
                            class="text-3xl sm:text-4xl font-bold bg-gradient-to-r from-purple-500 to-pink-500 bg-clip-text text-transparent animate-pulse"
                        >
                            Колесо Фортуны
                        </h1>
                        <p class="text-neutral-400 font-medium mb-8">
                            Боги решают судьбу...
                        </p>

                        <div
                            class="relative w-48 h-48 rounded-full border-4 border-purple-500 flex items-center justify-center bg-neutral-800 shadow-[0_0_20px_rgba(168,85,247,0.4)] overflow-hidden"
                        >
                            <div
                                class="absolute inset-0 bg-gradient-to-tr from-purple-500/20 to-pink-500/20 animate-spin"
                                style="animation-duration: 3s"
                            ></div>
                            <span
                                class="text-2xl font-bold text-neutral-100 z-10 transition-all duration-75 text-center px-4 break-words"
                            >
                    ...
                    <div v-else class="w-full flex flex-col gap-6">
                        ...
                        <div
                            class="w-full bg-neutral-900 border border-neutral-800 rounded-2xl p-6 shadow-xl flex flex-col gap-6"
                        >
                            <div
                                v-for="player in playersInfo.activePlayers"
                                :key="player.name"
                                class="flex justify-between items-center bg-neutral-800 p-3 rounded-xl"
                            >
                                ...
                                <div class="flex items-center gap-4">
                                    <span
                                        class="text-rose-400 font-bold text-lg"
                                        >{{ votes[player.name] }}</span
                                    >
                                    <SharedUiButton
                                        text="+ Голос"
                                        @click="recordVote(player.name)"
                            ...
                            <div
                                class="pt-4 border-t border-neutral-800 flex flex-col gap-3"
                            >
                                <SharedUiButton
                                    text="Завершить голосование"
                                    @click="resolveVoting"
                                    class="w-full !bg-rose-600"
                                />
                                <SharedUiButton
                                    text="Пропустить голосование"
                                    @click="skipVoting"
                    ...
                    >
                        <div class="flex items-start justify-between gap-4">
                            <div>
                                ...
                                <h2
                                    class="mt-2 text-2xl font-bold text-neutral-100"
                                >
                                    {{ getRoleTitle(currentNightRole) }}
                                </h2>
                                ...
                                <p
                                    v-if="
                                        isMafiaNightRole && mafiaTeamMembersText
                                    "
                                ...
                                >
                                    Мафия действует командой:
                                    {{ mafiaTeamMembersText }}. Игроков мафии
                                    нельзя выбрать целью.
                                </p>
                                <p
                                    v-else-if="currentRoleActor"
                                ...
                                >
                                    Ходит {{ currentRoleActor.name }}. Его
                                    нельзя выбрать целью.
                                </p>
                                <p
                        ...
                        <div
                            class="rounded-2xl border border-blue-400/10 bg-blue-400/5 p-4"
                        >
                            <div
                        ...
                        <div
                            class="pt-4 border-t border-neutral-800 flex flex-col sm:flex-row gap-3"
                        >
                            <SharedUiButton
                                class="w-full !bg-neutral-800 !text-neutral-400 hover:!bg-neutral-700"
                                :text="
                                    isSectarianNightRole
                                        ? 'Не выбирать цель'
                                        : 'Пропустить действие'
                                "
            ...
            <div v-if="playersInfo.isMasterMode" class="w-full">
                ...
                <Transition name="role-card">
                    ...
                    >
                        <div
                            v-for="(entry, role) in masterRoleGroups"
                            :key="role"
                            class="flex items-center justify-between gap-3 rounded-xl border px-4 py-2 transition-colors"
                            :class="entry.allDead
                                ? 'border-neutral-700 bg-neutral-800/30 opacity-50'
                                : 'border-neutral-800 bg-neutral-800/50'"
                        >
                            <span
                                class="font-semibold min-w-[90px]"
                                :class="entry.allDead ? 'text-neutral-500 line-through' : 'text-neutral-300'"
                            >{{ roleDisplayNames[role] || role }}</span>
                            <span class="text-neutral-400 text-sm text-right">{{ entry.names.join(", ") }}</span>
                        </div>
                    </div>
                </Transition>
            </div>
            ...
            <SharedUiModal :isModalVisible="isNightLogModalVisible">
                <div class="relative z-10 flex flex-col gap-5">
                    <div class="flex items-start justify-between gap-4">
                        <div>
                            <p
                                class="text-xs uppercase tracking-[0.3em] text-purple-300/70"
                            >
                                История
                            </p>
                            <h2
                            ...
                            <p class="mt-2 text-sm text-neutral-400">
                                Все действия, которые были записаны в последнюю
                                ночь.
                            </p>
                        </div>
                        <button
                    ...
                    <div
                        v-else
                        class="rounded-2xl border border-neutral-800 bg-neutral-900/80 p-5 text-center text-neutral-400"
                    >
                        Лог пуст.
                    </div>
                </div>
