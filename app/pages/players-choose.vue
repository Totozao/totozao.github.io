...
const allDone = computed(() => donePlayers.value.size === playersInfo.activePlayers.length);
...
<template>
  <NuxtLayout>
    <div class="flex flex-col items-center gap-8 w-full max-w-lg mx-auto py-8 px-4">
      ...
      <div class="text-center space-y-2">
        ...
        <p class="text-neutral-400 font-medium">
          {{ allDone ? 'Все роли розданы!' : `Осталось: ${remainingPlayers.length} из ${playersInfo.activePlayers.length}` }}
        </p>
      </div>
      ...
      <ClientOnly>
        ...
        >
          <p class="text-neutral-300 font-medium text-center">Кто смотрит первым?</p>
        ...
        <Transition name="flip" mode="out-in">
          <div
            ...
            :class="animating ? 'scale-95 opacity-0' : 'scale-100 opacity-100'"
            style="transition: transform 0.5s ease, opacity 0.5s ease"
          >
            <div
              class="flex flex-col items-center justify-center gap-6 p-10 bg-gradient-to-br text-white min-h-[320px]"
              :class="roleColors[revealedRole] || roleColors.civilian"
            >
              <p class="text-sm uppercase tracking-[0.3em] text-white/60">Ваша роль</p>
              <h2 class="text-5xl font-extrabold text-center drop-shadow-lg">
                {{ roleNames[revealedRole] || revealedRole }}
              </h2>
              <p class="text-white/70 text-sm text-center">Запомните свою роль и никому не показывайте экран.</p>
              <button
        ...
        >
          <div class="w-16 h-16 rounded-full bg-emerald-500/20 border border-emerald-400/30 flex items-center justify-center">
            <span class="text-3xl">✓</span>
          </div>
          <p class="text-neutral-200 font-semibold text-lg text-center">Роль получена. Передайте устройство следующему игроку.</p>
          <SharedUiButton text="Следующий игрок" class="w-full max-w-xs" @click="continueNext" />
        </div>
        ...
        >
          <h2 class="text-2xl font-bold text-emerald-400">Все роли розданы!</h2>
          <p class="text-neutral-400">Каждый игрок знает свою роль. Передайте устройство ведущему и начните игру.</p>
          <SharedUiButton
            text="Начать игру"
      ...
      <div class="flex gap-2 flex-wrap justify-center">
        <div
          ...
          :class="donePlayers.has(player.name) ? 'bg-emerald-400' : 'bg-neutral-700'"
          :title="player.name"
        />
      </div>
    </div>
